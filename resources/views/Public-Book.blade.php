...
::-webkit-scrollbar-thumb:hover {
    ...
    {{-- Book List Section --}}
    <section class="text-gray-600 body-font" _msthidden="9">
        {{-- <div class="px-5 py-24 mx-auto" _msthidden="9"> --}}
            <div class="flex flex-col gap-4 justify-center items-center p-4 my-6 mt-16">

                <h1 class="text-4xl text-center md:text-5xl lg:text-4xl font-bold mb-5" data-aos="zoom-in" data-aos-duration="3000">
                    Daftar Judul Buku
                </h1>
            
                <div id="search-bar" class="w-1/2 bg-white rounded-md border shadow-[4px_4px_4px_#b1b1b1]" data-aos="zoom-out" data-aos-duration="3000">
                    <form class="flex items-center justify-center p-2">
                        <input type="text" name="title" id="title" placeholder="Cari Buku"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-10 md:px-20">
                    @foreach ($books as $item)
                        <div class="bg-white rounded-xl overflow-hidden shadow-[4px_4px_4px_#b1b1b1]" data-aos="flip-right" data-aos-duration="3000">
                            <div class="relative">
                                <img class="w-full h-48 object-cover"
                                    src="{{ $item->cover != null ? asset('storage/cover/' . $item->cover) : asset('images/img-not-found.jpg') }}"
                                    draggable="false">
                                <div
                                    class="absolute top-0 right-0 text-white font-semibold px-2 py-1 m-2 rounded-md 
                            {{ $item->status == 'in stock' ? 'bg-green-500' : 'bg-red-500' }}">
                                    {{ $item->status }}
                                </div>
                            </div>
                            <div class="p-4">
                                <div class="text-lg font-medium text-gray-800 mb-3">{{ $item->title }}</div>
                                <p class="text-gray-500 text-sm text-end">Book Code : {{ $item->book_kode }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
